...
TeX Live 有两种安装方式：网络安装和镜像安装。

\subsubsection{网络安装}

网络安装需要联网，它会从网络上下载安装文件，因此安装过程会比较慢。

\subsubsection{镜像安装}

镜像安装则不需要联网，它会从本地镜像文件安装，因此安装过程会比较快。

我们推荐使用镜像安装，因为它不仅安装速度快，而且可以避免网络问题导致的安装失败。TeX Live 的镜像文件可以从\href{https://mirrors.ustc.edu.cn/CTAN/systems/texlive/Images/}{中科大开源镜像站}下载。我们推荐下载 \texttt{texlive.iso}，它是一个 ISO 镜像文件，可以使用 Windows 自带的虚拟光驱软件直接挂载并安装。如果你不知道如何挂载 ISO 镜像文件，请自行搜索。
...
\cite{chen2025kg}
\cite{wang2024event}
\cite{zhang2025continuous}
\cite{shahsavari2024event}
\cite{au2026mcmc}
\cite{kouloukoui2025ai}
\cite{han2025opinion}
\cite{zhang2025flight}
\cite{li2025genrec}
\cite{asri2026review}
\cite{wang2025kgrec}
\cite{tai2025learning}
\cite{jiang1989patent}
\cite{yao1990book}
\cite{xie1998news}
\cite{kyungmoon1998report}
\cite{pacs1989forum}，而这是一个上标形式的引用\supercite{chen2025kg}。

当然，类似于\texttt{\textbackslash cref}，我们也可以一次性引用多个参考文献。所有的参考文献都保存在主目录的\texttt{ref.bib}文件中，如果需要添加参考文献，只需要在该文件中添加即可。一般来说，我们可以在 DBLP等文献网站 中找到对应的参考文献，然后点击引用，选择 BibTeX 格式，选择相应的字段，复制粘贴到\texttt{ref.bib}文件中，注意格式调整，其格式如下：

\begin{lstlisting}[language=TeX]
% ref.bib
% Encoding: UTF8

@inproceedings{KDD2024Yang,
...
@article{MSSB2026Deng,
author  = {邓文 and 折延宏 and 郑文利 and 贺晓丽 and 钱婷},
title   = {基于高阶相关性与双重冗余驱动的全局多标签特征选择},
journal = {模式识别与人工智能},
volume  = {39},
number  = {01},
pages   = {52-66},
